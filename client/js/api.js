...
class ValetecApiClient {
  ...
  async getDashboardKpis() {
    return await this.request('/dashboard/kpis');
  }

  async getSalesChart() {
    return await this.request('/dashboard/sales-chart');
  }
  ...
  // 8.1. Configuraciones Globales de Empresa & Sistema
  async getSettings() {
  ...
  // 8.2. Generación y Descarga Segura de Backups PostgreSQL 16
  async downloadDatabaseBackup() {
    ...
    if (!response.ok) {
      let errMsg = 'Error al generar la copia de seguridad.';
      try {
    ...
    let filename = `valetec_pharma_backup_${new Date().toISOString().replace(/[:.]/g, '-')}.sql`;
